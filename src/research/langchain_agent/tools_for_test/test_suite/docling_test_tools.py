...
def _resolve_output_dir(output_dir: Optional[str], default_dir: Path) -> Path:
    return _ensure_directory(Path(output_dir).expanduser().resolve() if output_dir else default_dir)
...
def _extension_from_content_type(content_type: Optional[str]) -> str:
    ...
    override = _CONTENT_TYPE_EXTENSION_OVERRIDES.get(normalized)
    if override:
        return override
...
def _filename_from_response(url: str, response: httpx.Response, override_filename: Optional[str]) -> str:
    if override_filename:
        return _safe_filename(override_filename, "downloaded_file")

    content_disposition = response.headers.get("content-disposition", "")
    match = _CONTENT_DISPOSITION_FILENAME_RE.search(content_disposition)
    if match:
        return _safe_filename(unquote(match.group(1).strip()), "downloaded_file")

    parsed = urlparse(url)
    candidate = Path(unquote(parsed.path)).name
    if candidate:
        candidate = _safe_filename(candidate, "downloaded_file")
    else:
...
async def _download_file_impl(
    *,
    ...
    target_dir = _resolve_output_dir(output_dir, DEFAULT_DOWNLOAD_DIR)
    ...
    async def _download_once(verify_value: bool) -> tuple[Path, int, Optional[str], int]:
        async with httpx.AsyncClient(
            ...
            async with client.stream("GET", url) as response:
                ...
                resolved_filename = _filename_from_response(url, response, filename)
...
def _convert_local_file_sync(
    *,
    ...
    target_dir = _ensure_directory(Path(output_dir).expanduser().resolve())
    ...
    stem = _safe_filename(output_stem or source.stem, "converted_document")
...
async def _convert_local_file_impl(
    *,
    ...
    target_dir = _resolve_output_dir(output_dir, DEFAULT_CONVERSION_DIR)
    ...
    return await asyncio.to_thread(_convert_local_file_sync, **kwargs)
...
async def download_file_to_local(
    ...
    output_dir: Annotated[
        Optional[str],
        "Directory where the downloaded file should be saved. Defaults to test_suite/generated/downloads.",
    ] = None,
    filename: Annotated[
        Optional[str],
...
async def convert_local_file_with_docling(
    ...
    output_dir: Annotated[
        Optional[str],
        "Directory where the converted output should be written. Defaults to test_suite/generated/conversions.",
    ] = None,
...
async def batch_convert_local_files_with_docling(
    ...
    output_dir: Annotated[
        Optional[str],
        "Directory where converted outputs should be written. Defaults to test_suite/generated/conversions.",
    ] = None,
    use_multiprocessing: Annotated[
        bool,
